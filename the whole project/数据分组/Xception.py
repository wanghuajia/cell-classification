...
train_folders = ["./raw/train/cancer cell/", "./raw/train/lymphocyte/", "./raw/train/plasma cell/"]
...
valid_folders = ["./raw/validation/cancer cell/", "./raw/validation/lymphocyte/", "./raw/validation/plasma cell/"]
...
def get_callbacks(filepath,model,patience=4):
    ...
    msave = Mycbk(model, './Xception/'+filepath)
    file_dir = './Xception/log/'+ time.strftime('%Y_%m_%d',time.localtime(time.time()))
    ...
    log_cv = CSVLogger('./Xception/' + time.strftime('%Y_%m_%d',time.localtime(time.time())) +'_log.csv', separator=',', append=True)
    return [es, msave,reduce_lr,tb_log,log_cv]

import time
file_path = "Xception_decay.hdf5"
