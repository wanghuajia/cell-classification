...
base=load_model('InceptionV3(139).h5')
...
def get_callbacks(filepath,model,patience=4):
    ...
    msave = Mycbk(model, './InceptionV3/'+filepath)
    file_dir = './InceptionV3/log/'+ time.strftime('%Y_%m_%d',time.localtime(time.time()))
    ...
    log_cv = CSVLogger('./InceptionV3/' + time.strftime('%Y_%m_%d',time.localtime(time.time())) +'_log.csv', separator=',', append=True)
    return [es, msave,reduce_lr,tb_log,log_cv]

import time
file_path = "InceptionV3_decay.hdf5"
